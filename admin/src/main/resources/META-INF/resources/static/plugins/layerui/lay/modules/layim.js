...
layui.define(["layer", "laytpl", "upload"], function(i) {
	...
	var u = function(i) {
		...
		h = ['<li {{ d.mine ? "class=layim-chat-mine" : "" }} {{# if(d.cid){ }}data-cid="{{d.cid}}"{{# } }}>', '<div class="layim-chat-user"><img src="{{ d.avatar }}"><cite>', "{{# if(d.mine){ }}", '<i>{{ layui.data.date(d.timestamp) }}</i>{{ d.name||"佚名" }}', "{{# } else { }}", '{{ d.name||"佚名" }}<i>{{ layui.data.date(d.timestamp) }}</i>', "{{# } }}", "</cite></div>", '<div class="layim-chat-text">{{ layui.data.content(d.content||"&nbsp") }}</div>', "</li>"].join(""),
	...
	var x, b, w, k, C, S = function(i, a, n) {
			return i = i || {}, e.ajax({
				...
				error: function(i, a) {
					window.console && console.log && console.error("LAYIM_DATE_ERROR：" + a + "@" +i.url)
				}
			})
		},
		j = {
		...
		T = function(i) {
			return b && t.close(b.attr("times")), x && x.hide(), j.mine = j.mine || {}, t.open({
				...
				offset: "rb",
				resize: !1,
				content: '<img src="' + (j.mine.avatar || layui.cache.dir + "css/pc/layim/skin/logo.jpg") + '"><span>' + (i || j.base.title || "聊天") + "</span>",
		...
		$ = function() {
			var i = {
					username: j.mine ? j.mine.name : "访客",
		...
		K = function(i) {
			...
			if(i.timestamp = i.timestamp || (new Date).getTime(), i.fromid == j.mine.id && (i.mine = !0), i.system || B(i), G = JSON.parse(JSON.stringify(i)), j.base.voice && F(), !w && i.content || l === -1) {
				if(j.message[i.type + i.id]) j.message[i.type + i.id].push(i);
					...
					layui.each(j.friend, function(a, e) {
						if(layui.each(e.list, function(a, e) {
								if(e.id == i.id) return e.type = "friend", e.name = e.name, j.chat.push(e), s = !0
							}), s) return !0
					}), s || (i.name = i.name, i.temporary = !0, j.chat.push(i))
				...
				} else i.name = i.name || i.name || i.groupname, j.chat.push(i);
		...
		ti = {
					...
					n = function(i) {
						...
						if("" === a) ti.tab(0 | ti.tab.index);
							...
							for(var n = [], l = j.friend || [], s = j.group || [], o = "", d = 0; d < l.length; d++)
								for(var r = 0; r < (l[d].list || []).length; r++) l[d].list[r].name.indexOf(a) !== -1 && (l[d].list[r].type = "friend", l[d].list[r].index = d, l[d].list[r].list = r, n.push(l[d].list[r]));
							for(var c = 0; c < s.length; c++) s[c].groupname.indexOf(a) !== -1 && (s[c].type = "group", s[c].index = c, s[c].list = c, n.push(s[c]));
							if(n.length > 0)
								for(var u = 0; u < n.length; u++) o += '<li layim-event="chat" data-type="' + n[u].type + '" data-index="' + n[u].index + '" data-list="' + n[u].list + '"><img src="' + n[u].avatar + '"><span>' + (n[u].name || n[u].groupname || "佚名") + "</span><p>" + (n[u].remark || n[u].sign || "") + "</p></li>";
			...
			chat: function(i) {
				var a = layui.data("layim")[j.mine.id] || {},
					...
					l = {};
				"friend" === e ? l = j[e][t].list[n] : "group" === e ? l = j[e][n] : "history" === e && (l = (a.history || {})[t] || {}), l.name = l.name || l.name || l.groupname, "history" !== e && (l.type = e), z(l)
			},
			tabChat: function(i) {
				q(i)
			},
					...
					success: function(a) {
						...
						c && s.css({
							...
							layui.each(a.list, function(i, a) {
								o += '<li data-uid="' + a.id + '"><a href="javascript:;"><img src="' + a.avatar + '"><cite>' + a.name + "</cite></a></li>", d[a.id] = a
