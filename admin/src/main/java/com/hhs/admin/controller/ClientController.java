...
public class ClientController {
	...
	private final static String path = "admin/client/";
	...
	@RequiresPermissions("admin:client:add")
	...
	public Map<String, Object> addAction(Client model)
	{
	...
	@ResponseBody
	public Map<String, Object> updateAction(Client model)
	{
		Map<String, Object> map = new HashMap<>();
	...
	@RequiresPermissions("admin:client:show")
	...
	public String manager(ModelMap map, @PathVariable Integer pageIndex, @PathVariable Integer pageSize,String k)
	{
		...
		Page<Client> list = clientService.list(dc, pageIndex, pageSize);
		map.put("page", list);
		map.put("path", path);
		map.put("key", k);
		return path + "list";
	...
	@RequiresPermissions("admin:client:delete")
	@RequestMapping("deleteById/{id}.json")
	@ResponseBody
	public Map<String, Object> delete(@PathVariable Integer id)
	{
		Map<String, Object> map = new HashMap<>();
		if (clientService.deleteById(id, false)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
	...
	public Map<String, Object> delete(@PathVariable String ids)
	{
    	...
    	if (clientService.deleteByIds(arrayId, false)) {
    		map.put("status", "ok");
    	} else {
    		map.put("status", "no");
    	}
	...
	{
        Map<String, Object> map = new HashMap<>();
		if (clientService.update(p, v, id)) {
			map.put("status", "ok");
		} else {
			map.put("status", "no");
		}
