	                        ...
	                        <div class="box-body">
                                <div class="form-group">
									...
									<#if f.fieldName=='thumb' >
										<input type="hidden" id="baseThumb" name="baseThumb"
                                               value="${r"${model.baseThumb}"}">
                                    </#if>
                                        <input class="file" type="file" id='${f.fieldName!""}'> <input
                                            value="${r"${model."}${f.fieldName}}"
                          ...
                          < div class="col-sm-8" >
                              ...
                              <#if f.dataNature=='MODEL'>
												<div class="check-box ${f.fieldName!""}-admin-check-box">
                                                    <c:forEach items="${r"${"}${f.fieldName!""}List}"
                                                               var="${f.fieldName!""}Model">
                                                        <label><input value="${r"${"}${f.fieldName!""}Model.id}"
                                                                      name="${f.fieldName!""}" type="checkbox"
                                                                      class="minimal">${r"${"}${f.fieldName!""}
                                                            Model.name}</label>
                                                    </c:forEach>
                                                </div>
                              </#if>
									...
									<div class="form-group">
                                        ...
                                        <div class="col-sm-8">
											<#if f.dataNature=='ARRAY' >
                                                <#if f.dataArray?? && f.dataArray?size gt 0>
                                                    <#list f.dataArray?keys as key  >
											<label><input value="${key}"  ${r"${model."}${f.fieldName}==${key}
                                                ?'checked':'' } type="radio" class="minimal" name="${f.fieldName}"
                                                >${f.dataArray[key]!}</label>
                                                    </#list>
                                                </#if>
                                            </#if>
											<#if f.dataNature=='MODEL'>
											<c:forEach items="${r"${"}${f.fieldName!""}List}"
                                                       var="${f.fieldName!""}Model">
                                                <label><input value="${r"${"}${f.fieldName!""}Model.id}" type="radio"
                                                              class="minimal"
                                                              name="${f.fieldName!""}">${f.dataArray[key]!}</label>
                                            </c:forEach>
                                            </#if>
                                        </div>
                                    </div>
                          ...
                          <#if f.type=='TEXTAREA' >
									<div class="form-group">
                                        <label class="col-sm-2 control-label"><#if f.datatype??><#if f.datatype!=""><i
                                                class="red">*</i></#if></#if>${f.lable!""}:</label>
                                        <div class="col-sm-8">
                                            <textarea name="${f.fieldName!""}" class="form-control"
                                                      id="${f.fieldName!""}" rows="3" placeholder="Enter ..."
                                                      <#if f.datatype?? && f.datatype!="">datatype="${f.datatype}"</#if>
                                                      <#if f.errormsg?? && f.errormsg!="">errormsg="${f.errormsg}"</#if>
                                                      <#if f.recheck?? && f.recheck!="">recheck="${f.recheck}"</#if>
                                                      <#if f.ajaxurl?? && f.ajaxurl!="">ajaxurl="${f.ajaxurl}"</#if>
